...
import AuthorizationMigration from '../src/models/AuthorizationMigration.js';
...
test('MongoDB roles, permissions and resource authorization', { skip: !process.env.MONGODB_URI }, async (t) => {
  ...
  await t.test('stable role references, unique IDs and non-destructive initialization', async () => {
    ...
    assert.ok((await Role.findById('guardian')).permissions.includes('guardian.view'));
    assert.ok((await Role.findById('teacher')).permissions.includes('enrollment.view'));
    assert.ok((await Role.findById('student')).permissions.includes('enrollment.view'));
    assert.ok((await Role.findById('guardian')).permissions.includes('enrollment.view'));
    assert.ok((await Role.findById('teacher')).permissions.includes('teacher_assignment.view'));
    ...
    assert.ok((await Role.findById('admin')).permissions.includes('academic_year.view'));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase7-academic-year-admin-permissions' }));
    assert.ok((await Role.findById('admin')).permissions.includes('academic_structure.view'));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase8-academic-structure-admin-permissions' }));
    assert.ok((await Role.findById('admin')).permissions.includes('teacher.create'));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase10-teacher-admin-permissions' }));
    assert.ok((await Role.findById('admin')).permissions.includes('guardian.create'));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase11-guardian-permissions' }));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase11-guardian-self-view-permission' }));
    assert.ok((await Role.findById('admin')).permissions.includes('enrollment.create'));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase12-enrollment-admin-permissions' }));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase12-enrollment-teacher-view-permission' }));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase12-enrollment-student-view-permission' }));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase12-enrollment-guardian-view-permission' }));
    assert.ok((await Role.findById('admin')).permissions.includes('teacher_assignment.create'));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase13-teacher-assignment-admin-permissions' }));
    assert.ok(await AuthorizationMigration.exists({ _id: 'phase13-teacher-assignment-teacher-view-permission' }));
    await Role.updateOne({ _id: 'admin' }, { $pull: { permissions: 'academic_year.view' } });
    await Role.updateOne({ _id: 'admin' }, { $pull: { permissions: 'academic_structure.view' } });
    await Role.updateOne({ _id: 'admin' }, { $pull: { permissions: 'teacher.create' } });
    await Role.updateOne({ _id: 'admin' }, { $pull: { permissions: 'guardian.create' } });
    await Role.updateOne({ _id: 'admin' }, { $pull: { permissions: 'enrollment.create' } });
    await Role.updateOne({ _id: 'admin' }, { $pull: { permissions: 'teacher_assignment.create' } });
    await Role.updateOne({ _id: 'teacher' }, { $pull: { permissions: 'enrollment.view' } });
    await Role.updateOne({ _id: 'teacher' }, { $pull: { permissions: 'teacher_assignment.view' } });
    await Role.updateOne({ _id: 'student' }, { $pull: { permissions: 'enrollment.view' } });
    await Role.updateOne({ _id: 'guardian' }, { $pull: { permissions: 'enrollment.view' } });
    await Role.updateOne({ _id: 'guardian' }, { $pull: { permissions: 'guardian.view' } });
    await seedAuthorization();
    assert.ok(!(await Role.findById('admin')).permissions.includes('academic_year.view'));
    assert.ok(!(await Role.findById('admin')).permissions.includes('academic_structure.view'));
    assert.ok(!(await Role.findById('admin')).permissions.includes('teacher.create'));
    ...
    assert.ok(!(await Role.findById('guardian')).permissions.includes('enrollment.view'));
    assert.ok(!(await Role.findById('guardian')).permissions.includes('guardian.view'));
    await Role.updateOne({ _id: 'admin' }, { $addToSet: { permissions: 'academic_year.view' } });
    await Role.updateOne({ _id: 'admin' }, { $addToSet: { permissions: 'academic_structure.view' } });
    await Role.updateOne({ _id: 'admin' }, { $addToSet: { permissions: 'teacher.create' } });
    await Role.updateOne({ _id: 'admin' }, { $addToSet: { permissions: 'guardian.create' } });
    await Role.updateOne({ _id: 'admin' }, { $addToSet: { permissions: 'enrollment.create' } });
    await Role.updateOne({ _id: 'admin' }, { $addToSet: { permissions: 'teacher_assignment.create' } });
    await Role.updateOne({ _id: 'teacher' }, { $addToSet: { permissions: 'enrollment.view' } });
    await Role.updateOne({ _id: 'teacher' }, { $addToSet: { permissions: 'teacher_assignment.view' } });
    await Role.updateOne({ _id: 'student' }, { $addToSet: { permissions: 'enrollment.view' } });
    await Role.updateOne({ _id: 'guardian' }, { $addToSet: { permissions: 'enrollment.view' } });
    await Role.updateOne({ _id: 'guardian' }, { $addToSet: { permissions: 'guardian.view' } });
